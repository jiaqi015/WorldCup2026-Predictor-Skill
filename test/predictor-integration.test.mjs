...
test("simulated group and knockout matches open a unified timeline detail modal", () => {
  ...
  assert.match(html, /detailSourceOfficial:"官方赛程 · ESPN"/);
  ...
  assert.match(html, /\.match-detail-shell\{[^}]*grid-template-columns:288px minmax\(0,1fr\)/);
...
test("stats and share poster expose tournament awards derived from match events", () => {
  ...
  assert.match(html, /\.award-logo \.award-metal\{fill:currentColor\}/);
  assert.match(html, /class="award-metal"/);
  assert.match(html, /class="award-highlight"/);
  assert.match(html, /class="award-line"/);
  assert.match(html, /viewBox="0 0 64 64"/);
  assert.doesNotMatch(html, /class="award-fill"/);
...
test("fresh live scores hydrate missing ESPN summary goal events at runtime", () => {
  ...
  assert.match(html, /function actualMatchScore\(actual,side\)/);
...
test("knockout cards show source groups only in round-of-32 metadata", () => {
  ...
  assert.match(html, /function getOfficialKOMatchFixture\(id\)/);
  assert.match(html, /function officialFixtureStatusLabel\(fixture,actual\)/);
  assert.match(html, /function knockoutDetailEmptyText\(fixture,actual,rec\)/);
